...
class PreTrainedTokenizer(object):
        ...
        for key, value in kwargs.items():
            if key in self.SPECIAL_TOKENS_ATTRIBUTES:
                ...
                if key == 'additional_special_tokens':
                    assert isinstance(value, (list, tuple)) and all(isinstance(t, str) or (six.PY2 and isinstance(t, str)) for t in value)
                else:
    ...
    def _from_pretrained(cls, pretrained_model_name_or_path, *init_inputs, **kwargs):
        ...
        print(3,init_inputs)
        print(4,init_kwargs)
        keys_to_keep = {'max_len', 'vocab_file', 'merges_file'}
        dictionary_keys = set(init_kwargs.keys())
        keys_to_remove = dictionary_keys - keys_to_keep
