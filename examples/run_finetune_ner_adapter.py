...
from utils_ner import (compute_metrics, convert_examples_to_features_entity_typing,
...
class PretrainedModel(nn.Module):
    def __init__(self, model_name, args,num_labels):
...
def main():
    ...
    ## editar para sacar las labels necesarias
    ...
    config = config_class.from_pretrained(model_path, num_labels=num_labels, finetuning_task=args.task_name) #args.config_name if args.config_name else args.model_name_or_path
    tokenizer = tokenizer_class.from_pretrained(model_path, do_lower_case=args.do_lower_case)  #args.config_name if args.config_name else args.model_name_or_path
    # model = model_class.from_pretrained(args.model_name_or_path, from_tf=bool('.ckpt' in args.model_name_or_path), config=config)

    #tokenizer = RobertaTokenizer.from_pretrained('roberta-large')
    tokenizer = RobertaTokenizer.from_pretrained(model_path)
    pretrained_model = PretrainedModel(model_path,args, num_labels)
    ...
    figer_model = NERModel(args, pretrained_model.config, fac_adapter=fac_adapter, et_adapter=et_adapter, lin_adapter=lin_adapter)
