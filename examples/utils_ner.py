...
class InputNERFeatures(object):
    """A single set of features of data."""

    def __init__(self, input_ids, input_mask, segment_ids, labels_id):
        self.input_ids = input_ids
        self.input_mask = input_mask
        self.segment_ids = segment_ids
        self.label_id = labels_id
...
class NERTypeProcessor(DataProcessor):
    ...
    for i, label in enumerate(tags):
        ...
        for word_idx in word_ids:
            if word_idx is None:
                ...
                label_ids.append(label_map[label[word_idx]])  # (label2id[label[word_idx]])
            else:
                label_ids.append(label_map[label[word_idx]] if label_all_tokens else -100)  # (label2id[label[word_idx]] if label_all_tokens else -100)
            previous_word_idx = word_idx
        labels.append(label_ids)

    tokenized_inputs["labels"] = labels
    return tokenized_inputs['input_ids'][0], tokenized_inputs['attention_mask'][0], tokenized_inputs['labels'][0]
...
def convert_examples_to_features_entity_typing(examples, label_list, max_seq_length,
    ...
    tokenizer = AutoTokenizer.from_pretrained(model_path, add_prefix_space=True, truncation=True, use_fast=True,
    ...
    for (ex_index, example) in enumerate(examples):
        ...
        input_ids, attention_mask, labels_ids = tokenize_and_align_labels(sentences, labels, tokenizer,label_map)
        ...
        if ex_index < 5:
            ...
            logger.info("tokens: %s" % " ".join(
                [str(x) for x in sentences]))
            logger.info("input_ids: %s" % " ".join([str(x) for x in input_ids]))
            logger.info("input_mask: %s" % " ".join([str(x) for x in input_mask]))
            logger.info("segment_ids: %s" % " ".join([str(x) for x in segment_ids]))
            logger.info("label: {}".format(labels_ids))




        features.append(
            InputNERFeatures(input_ids=input_ids,
